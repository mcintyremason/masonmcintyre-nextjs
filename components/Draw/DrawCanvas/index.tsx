...
const DrawCanvas: React.FC<DrawCanvasProps> = (props: DrawCanvasProps) => {
  const {
    width = 600,
    height = 600,
        ...
        if (moving) {
          if (selectedShapeType != null) {
            ...
            currentShape.centerX =
              currentShape.firstX + (currentShape.x - currentShape.firstX) / 2;
            currentShape.centerY =
              currentShape.firstY + (currentShape.y - currentShape.firstY) / 2;
          }
        }
      ...
      for (let i = 0; i < shapes.length; i++) {
        ...
        if (shapes[i].selected === true) {
          ...
          if (shapes[i].firstX < shapes[i].x && shapes[i].firstY > shapes[i].y)
            ...
            shapes[i].firstX > shapes[i].x &&
            shapes[i].firstY > shapes[i].y
          )
            ctx.strokeRect(
              shapes[i].firstX + 5,
              shapes[i].firstY + 5,
  ...
  const load = () => {
    ...
    setReDraw(true);
    var loadname = prompt(
      "Please enter the name of the file wish to load.",
      "default",
    );
